    ...
    if IS_CATEGORY_SET:
        ...
        try:
            dfBasedOnUserReq = usr.setting_cat_by_usrReq(USER_REQUEST_TEXT_BOX_MAKE, USER_REQUEST_TEXT_BOX_MODEL,
                                                         CATEGORY_TYPE)
            if IS_FILTER_SET:
                dfBasedOnUserReq = usr.filter_by_usrReq(dfBasedOnUserReq, FILTER_NAME)
...
def set_filter():
    global FILTER_NAME
    FILTER_NAME = textbox_filter.get(1.0, "end-1c")
    global IS_FILTER_SET
    IS_FILTER_SET = True
    messagebox.showinfo("showinfo", "Filter is been updated")
...
btn_set_filter = tk.Button(frameUser, text='Filter', command=set_filter)
textbox_filter = tk.Text(frameUser, height=1.4, width=10)
label_filter = tk.Label(frameUser, text="Filter - ")
...
n1 = 0.1
# Filter
textbox_filter.place(relx=0.4, rely=0.37+n1)
label_filter.place(relx=0.02, rely=0.37+n1)
btn_set_filter.place(relx=0.02, rely=0.4+n1)
